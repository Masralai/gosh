...
import (
	...
	"github.com/Masralai/gosh/internal/readline/inputrc"
	"github.com/Masralai/gosh/internal/readline/internal/completion"
	"github.com/Masralai/gosh/internal/readline/internal/core"
	"github.com/Masralai/gosh/internal/readline/internal/display"
	"github.com/Masralai/gosh/internal/readline/internal/editor"
	"github.com/Masralai/gosh/internal/readline/internal/history"
	"github.com/Masralai/gosh/internal/readline/internal/keymap"
	"github.com/Masralai/gosh/internal/readline/internal/macro"
	"github.com/Masralai/gosh/internal/readline/internal/term"
	"github.com/Masralai/gosh/internal/readline/internal/ui"
)
...
type Shell struct {
	...
	// PostDisplay is a callback that returns ghost text to display inline
	// after the cursor position (zsh-style autosuggestions).
	// The function receives the current input line and returns the
	// suggestion text to display after the cursor in dimmed style.
	// Return empty string to show no ghost text.
	PostDisplay func(line []rune) string
...
func NewShell(opts ...inputrc.Option) *Shell {
	...
	// Initialize InsertText with cursor insert functionality
	shell.InsertText = func(text string) {
		cursor.InsertAt([]rune(text)...)
	}
...
func (rl *Shell) PrintTransientf(msg string, args ...any) (n int, err error) {
	...
	// clear everything below (prompt/line/completions).
...
// SetPostDisplay sets the PostDisplay callback for inline ghost text (zsh-style).
// The callback receives the current line and returns ghost text to display after cursor.
func (rl *Shell) SetPostDisplay(postDisplay func(line []rune) string) {
	rl.PostDisplay = postDisplay
	rl.Display.SetPostDisplay(postDisplay)
}
