...
func (e *Engine) displayLineRefactored() {
	...
	// Get PostDisplay ghost text (zsh-style inline autosuggestions)
	var ghostSuffix string
	if e.postDisplay != nil {
		ghostSuffix = e.postDisplay(*e.line)
	}

	// Get the subset of the suggested line to print.
	// Priority: PostDisplay > history-autosuggest > none
	if ghostSuffix != "" {
		line += color.Dim + ghostSuffix + color.Reset
